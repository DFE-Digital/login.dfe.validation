...
describe('validation library', () => {
    it('validate - should call next if no errors', async () => {
        ...
        let jsonFn = jest.fn();
        let res = { status: jest.fn().mockImplementation((statusCode) => ({ json: jsonFn })) };
        let next = jest.fn();
    ...
    it('validate - should call callback with errors if provided', async () => {
        ...
        expect(callback).toHaveBeenCalledWith("{\"validationErrors\":[{\"name\":\"id\",\"message\":\"expected a UUID\"}],\"body\":null}")
    });
    it('validate - should call callback with errors and body if cb provided with loggerIncludeBody', async () => {
        ...
        expect(callback).toHaveBeenCalledWith("{\"validationErrors\":[{\"name\":\"id\",\"message\":\"expected a UUID\"}],\"body\":{\"id\":\"not-a-uuid\"}}")
    });
});